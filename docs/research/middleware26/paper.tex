...
Our central industrial finding is clear: graph analysis is not the computational bottleneck. Across five scaling operational naval platform profiles, static verification executes in 15.53\,s on a single-node airborne profile and 100.16\,s on the largest combatant profile (P95 102.31\,s), representing under 5.6\% of an average 30-minute CI/CD pipeline that scales with application count. The rule catalog targets pub/sub misconfiguration classes that would otherwise surface only on physical testbeds. We report the framework architecture, multi-scale verification latency, the misconfiguration classes the catalog targets, and practical lessons from building the prototype inside a defense engineering organization.
\end{abstract}
...
\begin{enumerate}\itemsep=0.5pt
    ...
    \item \textbf{Verification Complexity \& Industrial Bottleneck Analysis (\S5, \S6):} Benchmarks across five industrial-scale naval platform profiles (1,498--3,254 components; 1--66 nodes) representative of modern combat management system deployments show gate latency of 15.53--100.16\,s (P95 $\le 102.31$\,s), representing under 5.6\% of an average 30-minute naval CI/CD pipeline whose duration scales with the number of integrated applications. We document that 6 of 7 specified verification capabilities are blocked by configuration data acquisition across defense engineering silos, not by algorithmic complexity.
\end{enumerate}
...
\section{Multi-Scale Naval Platform Architectures \& Verification Methodology} To assess the practical applicability of SaaG pre-deployment verification in an operational defense-system environment, we model the architectural scales and middleware configurations of five representative platform profiles spanning airborne, coastal surveillance, unmanned, surface-combatant, and task-group command domains. \subsection{Multi-Domain Combat System Architecture \& Platform Profiles} Modern combat-system ecosystems are deployed across diverse physical and operational domains. To capture this diversity, we construct five representative platform profiles covering airborne command-and-control, land-based surveillance, autonomous systems, surface combatants, and large-scale command platforms. The \textbf{Maritime Surveillance Aircraft} profile represents maritime patrol aircraft, helicopters, and airborne unmanned platforms that integrate radar processing, acoustic sensing, and tactical data-link relay within a compact single-node deployment ($|V|=1{,}498$, $|E|=4{,}824$). The \textbf{Coastal Surveillance} profile represents multi-sensor maritime picture compilation centers integrating radar, AIS, ADS-B, and electro-optical surveillance across distributed infrastructure ($|V|=2{,}174$, $|E|=7{,}142$) deployed on 23 server nodes. The \textbf{Unmanned Sea Surface Vehicle} profile represents mission-management architectures for unmanned surface, underwater, and aerial vehicles, including interoperability requirements aligned with STANAG 4586~\cite{stanag4586} ($|V|=2{,}192$, $|E|=6{,}980$). The \textbf{Amphibious Ship} profile represents task-group and joint-operations command environments with multi-link coordination and planning capabilities spanning 66 consoles and server chassis ($|V|=2{,}775$, $|E|=9{,}620$). The \textbf{Corvette Ship} profile represents multi-warfare combat management deployments supporting anti-air, anti-surface, anti-submarine, and electronic warfare functions across distributed operator consoles ($|V|=3{,}254$, $|E|=11{,}460$).
...
\begin{table}[b] \centering \caption{Gate Execution Latency Across Five Representative Platform Profiles ($N=500$).} \label{tab:latency} \small \setlength{\tabcolsep}{2.2pt} \begin{tabularx}{\columnwidth}{X r r r r r r} \toprule \textbf{Profile} & \textbf{Nodes} & \textbf{Comp.} & \textbf{Constr.} & \textbf{Anal.} & \textbf{Total} & \textbf{P95} \\ & & & (s) & (s) & (s) & (s) \\ \midrule Maritime Surveillance Aircraft & 1 & 1498 & 11.110 & 4.416 & \textbf{15.526} & 17.270 \\ Coastal Surveillance & 23 & 2174 & 24.493 & 14.071 & \textbf{38.565} & 38.935 \\ Unmanned Sea Surface Vehicle & 1 & 2192 & 23.525 & 21.898 & \textbf{45.423} & 47.625 \\ Amphibious Ship  & 66 & 2775 & 44.521 & 41.345 & \textbf{85.866} & 87.677 \\ Corvette Ship & 21 & 3254 & 52.344 & 47.813 & \textbf{100.157} & 102.312 \\ \bottomrule \end{tabularx} \end{table}
...
\begin{itemize}\itemsep=0.5pt
\item \textbf{Dominance of Graph Construction:} Graph construction accounts for \textbf{51.8\% to 71.6\% of total wall-clock time} across all platform profiles (e.g., 11.11\,s out of 15.53\,s for Maritime Surveillance Aircraft, and 52.34\,s out of 100.16\,s for Corvette Ship), while pure graph analysis and rule evaluation account for the remaining \textbf{28.4\% to 48.2\%} of total execution time (4.42\,s to 47.81\,s).
\item \textbf{Predictable Latency Scaling with Topological Complexity:} Verification latency scales monotonically with overall component count and dependency graph density, ranging from \textbf{15.53\,s} (P95 17.27\,s) for the compact 1,498-component Maritime Surveillance Aircraft profile up to \textbf{100.16\,s} (P95 102.31\,s) for the 3,254-component Corvette Ship deployment. Distributed multi-node configurations such as the 66-node Amphibious Ship (85.87\,s, P95 87.68\,s) incur substantial dependency projection and cross-node lifting overhead, yet overall verification latency remains well within practical CI/CD runtime tolerances ($<1.7$\,minutes).
\end{itemize}
In operational naval software engineering environments, complete CI/CD pipeline execution averages approximately 30 minutes, scaling directly with the number of integrated applications (due to compilation, unit test suites, and container packaging across 184--318 binaries). Across all operational configurations, total pipeline gating latency remains $\le 100.2$\,s (P95 $\le 102.3$\,s, $\le 5.6\%$ of overall pipeline time), confirming that static architectural verification is practical for pre-deployment gating in naval CI/CD pipelines without introducing prohibitive delays.
